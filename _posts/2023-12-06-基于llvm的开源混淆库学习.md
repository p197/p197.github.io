...
判断真实块，return块的方式与前面相同，模拟的过程就相当于在向下遍历一棵树`（但是因为循环的存在，所以称为有向图更合适。）`，整体的思路为：
...
1. return的代码块只识别记录了一个，有些复杂的代码可能存在多个return的代码块。
...
def hook_code(uc, address, size, user_data):
    ...
    for ins in md.disasm(bin[address-base:address - base + size], address-base):
        ...
        if ins.id == X86_INS_RET and cur_start_addr == retn_node.addr:
            is_success = False
            uc.emu_stop()
            return False
...
if __name__ == '__main__':
    ...
    relevants = {}
    for node in list(relevant_nodes) + [prologue_node] + [retn_node]:
